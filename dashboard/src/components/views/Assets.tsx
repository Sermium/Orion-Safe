    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md mx-4 shadow-2xl">
        ...
        <div className="bg-slate-800/50 rounded-xl p-4 mb-6 border border-slate-700">
          <div className="text-sm text-gray-400 mb-1">{t('assets.availableBalance')}</div>
        ...
        <div className="space-y-3">
          ...
          <button onClick={() => onDeposit(asset)} className="w-full flex items-center gap-4 p-4 rounded-xl bg-green-500/10 border border-green-500/30 hover:bg-green-500/20 hover:border-green-500/50 transition-all group">
            ...
            <div className="text-left">
              <div className="font-semibold text-white">{t('assets.actions.deposit')}</div>
              <div className="text-sm text-gray-400">{t('assets.actions.depositDesc', { symbol: asset.symbol })}</div>
            </div>
          </button>
          ...
          {canExecute && (
            <button onClick={() => onSend(asset)} className="w-full flex items-center gap-4 p-4 rounded-xl bg-blue-500/10 border border-blue-500/30 hover:bg-blue-500/20 hover:border-blue-500/50 transition-all group">
              ...
              <div className="text-left">
                <div className="font-semibold text-white">{t('assets.actions.send')}</div>
                <div className="text-sm text-gray-400">{t('assets.actions.sendDesc', { symbol: asset.symbol })}</div>
              </div>
            </button>
          )}
          ...
          {isAdmin && (
            <button onClick={() => onLock(asset)} className="w-full flex items-center gap-4 p-4 rounded-xl bg-purple-500/10 border border-purple-500/30 hover:bg-purple-500/20 hover:border-purple-500/50 transition-all group">
              ...
              <div className="text-left">
                <div className="font-semibold text-white">{t('assets.actions.timeLock')}</div>
                <div className="text-sm text-gray-400">{t('assets.actions.timeLockDesc')}</div>
              </div>
            </button>
          )}
          ...
          {isAdmin && (
            <button onClick={() => onVesting(asset)} className="w-full flex items-center gap-4 p-4 rounded-xl bg-indigo-500/10 border border-indigo-500/30 hover:bg-indigo-500/20 hover:border-indigo-500/50 transition-all group">
              ...
              <div className="text-left">
                <div className="font-semibold text-white">{t('assets.actions.vesting')}</div>
                <div className="text-sm text-gray-400">{t('assets.actions.vestingDesc')}</div>
              </div>
            </button>
          )}
        </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-1">{t('assets.subtitle')}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-800/50 rounded-xl p-5 border border-slate-700">
          <div className="text-gray-400 text-sm mb-1">{t('assets.totalBalance')}</div>
          <div className="text-2xl font-bold">{totalBalance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 4 })}</div>
        </div>
        <div className="bg-slate-800/50 rounded-xl p-5 border border-slate-700">
          <div className="text-gray-400 text-sm mb-1">{t('assets.title')}</div>
          <div className="text-2xl font-bold">{vaultBalance.length}</div>
        </div>
        <div className="bg-slate-800/50 rounded-xl p-5 border border-slate-700">
          <div className="text-gray-400 text-sm mb-1">{t('assets.remainingDailySpend')}</div>
      ...
      <div className="bg-slate-800/50 rounded-xl border border-slate-700 overflow-hidden">
        <div className="p-4 border-b border-slate-700 flex items-center justify-between">
          <h2 className="font-semibold">{t('assets.yourAssets')}</h2>
          <span className="text-sm text-gray-400">{t('assets.clickToManage')}</span>
        </div>
        ...
        {vaultBalance.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-300">{t('assets.noAssets')}</h3>
            <p className="text-gray-500 mt-1 mb-4">{t('assets.noAssetsDesc')}</p>
