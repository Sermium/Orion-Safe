...
const PublicVaultView: React.FC<PublicVaultViewProps> = ({ vaultAddress, onClose }) => {
  ...
  const loadVaultData = async () => {
    ...
    try {
      ...
      setError(err.message || t('publicVault.errors.loadFailed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  // --- Logical string helpers (DO NOT translate; used for colors/comparisons) ---
  ...
  // --- Display label helpers (translated) ---
  const getStatusLabel = (status: any): string => {
    switch (getStatusString(status).toLowerCase()) {
      case 'active': return t('publicVault.status.active');
      case 'partiallyreleased': return t('publicVault.status.partiallyReleased');
      case 'fullyreleased': return t('publicVault.status.fullyReleased');
      case 'cancelled': return t('publicVault.status.cancelled');
      default: return t('common.unknown');
    }
  };

  const getLockTypeLabel = (lockType: any): string => {
    return getLockTypeString(lockType) === 'TimeLock'
      ? t('publicVault.lockType.timeLock')
      : t('publicVault.lockType.vesting');
  };
  ...
  const getTimeRemaining = (lock: Lock): string => {
    try {
      ...
      if (getLockTypeString(lock.lock_type) === 'TimeLock') {
        if (now >= endTime) return t('publicVault.time.unlocked');
        const remaining = endTime - now;
        const days = Math.floor(remaining / 86400);
        const hours = Math.floor((remaining % 86400) / 3600);
        return t('publicVault.time.remaining', { days, hours });
      } else {
        if (now >= cliffTime) return t('publicVault.time.cliffPassed');
        const remaining = cliffTime - now;
        const days = Math.floor(remaining / 86400);
        const hours = Math.floor((remaining % 86400) / 3600);
        return t('publicVault.time.toCliff', { days, hours });
      }
    } catch {
  ...
  // Tab label map (keeps the typed union, translates the display text).
  const tabLabels: Record<'overview' | 'locks' | 'calendar' | 'assets', string> = {
    overview: t('publicVault.tabs.overview'),
    locks: t('publicVault.tabs.locks'),
    calendar: t('publicVault.tabs.calendar'),
    assets: t('publicVault.tabs.assets'),
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400">{t('publicVault.loading')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center">
        <div className="bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-red-500/30 p-8 max-w-md text-center">
          ...
          <h2 className="text-xl font-bold text-white mb-2">{t('publicVault.errors.title')}</h2>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-white">{config?.name || t('publicVault.defaultName')}</h1>
            ...
            <div className="flex items-center space-x-3">
              ...
              >
                ...
                <span className="text-gray-300 text-sm">{t('common.share')}</span>
              </button>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-slate-800/50 backdrop-blur-xl rounded-xl border border-white/10 p-6">
            <p className="text-gray-400 text-sm mb-1">{t('publicVault.stats.signers')}</p>
            <p className="text-2xl font-bold text-white">{signers.length}</p>
            <p className="text-gray-500 text-xs mt-1">{t('publicVault.stats.threshold', { count: config?.threshold || 1 })}</p>
          </div>
          <div className="bg-slate-800/50 backdrop-blur-xl rounded-xl border border-white/10 p-6">
            <p className="text-gray-400 text-sm mb-1">{t('publicVault.stats.totalLocks')}</p>
            <p className="text-2xl font-bold text-white">{locks.length}</p>
            <p className="text-gray-500 text-xs mt-1">{t('publicVault.stats.activeCount', { count: activeLocks.length })}</p>
          </div>
          <div className="bg-slate-800/50 backdrop-blur-xl rounded-xl border border-white/10 p-6">
            <p className="text-gray-400 text-sm mb-1">{t('publicVault.stats.assets')}</p>
            <p className="text-2xl font-bold text-white">{balances.filter(b => safeBigInt(b.balance) > BigInt(0)).length}</p>
            <p className="text-gray-500 text-xs mt-1">{t('publicVault.stats.tokensHeld')}</p>
          </div>
          <div className="bg-slate-800/50 backdrop-blur-xl rounded-xl border border-white/10 p-6">
            <p className="text-gray-400 text-sm mb-1">{t('publicVault.stats.lockedValue')}</p>
            <p className="text-2xl font-bold text-white">{formatAmount(totalLockedValue)}</p>
            <p className="text-gray-500 text-xs mt-1">{t('publicVault.stats.totalLocked')}</p>
          </div>
        </div>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            {/* Signers */}
            <div className="bg-slate-800/50 backdrop-blur-xl rounded-xl border border-white/10 p-6">
              <h3 className="text-lg font-semibold text-white mb-4">{t('publicVault.sections.signers')}</h3>
            ...
            {/* Recent Locks */}
            <div className="bg-slate-800/50 backdrop-blur-xl rounded-xl border border-white/10 p-6">
              <h3 className="text-lg font-semibold text-white mb-4">{t('publicVault.sections.recentLocks')}</h3>
              {locks.length === 0 ? (
                <p className="text-gray-400 text-center py-8">{t('publicVault.noLocks')}</p>
              ) : (
                <div className="space-y-3">
                    ...
                    <div key={lock.id} className="flex items-center justify-between p-4 bg-slate-700/30 rounded-lg">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          ...
                          <p className="text-gray-400 text-sm">{t('publicVault.toBeneficiary', { address: truncateAddress(lock.beneficiary) })}</p>
                        </div>
                      </div>
        ...
        {activeTab === 'locks' && (
          <div className="bg-slate-800/50 backdrop-blur-xl rounded-xl border border-white/10 p-6">
            <h3 className="text-lg font-semibold text-white mb-4">{t('publicVault.allLocks', { count: locks.length })}</h3>
            {locks.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-400">{t('publicVault.noLocks')}</p>
              </div>
            ) : (
                ...
                {locks.map((lock) => (
                  <div key={lock.id} className="bg-slate-700/30 rounded-xl p-5 border border-white/5">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                      <div>
                        <p className="text-gray-500 text-xs mb-1">{t('publicVault.fields.amount')}</p>
                        <p className="text-white font-medium">
                          {formatAmount(lock.total_amount, getTokenDecimals(lock.token))} {getTokenSymbol(lock.token)}
                        </p>
                      </div>
                      <div>
                        <p className="text-gray-500 text-xs mb-1">{t('publicVault.fields.beneficiary')}</p>
                        <p className="text-gray-300 font-mono text-sm">{truncateAddress(lock.beneficiary)}</p>
                      </div>
                      <div>
                        <p className="text-gray-500 text-xs mb-1">{t('publicVault.fields.unlockDate')}</p>
                        <p className="text-gray-300 text-sm">
                          {new Date(safeNumber(lock.end_time) * 1000).toLocaleDateString(locale)}
                        </p>
                      </div>
                      <div>
                        <p className="text-gray-500 text-xs mb-1">{t('publicVault.fields.timeRemaining')}</p>
                    ...
                    <div className="mt-4">
                      <div className="flex justify-between text-xs text-gray-500 mb-1">
                        <span>{t('publicVault.released')}</span>
        ...
        {activeTab === 'assets' && (
          <div className="bg-slate-800/50 backdrop-blur-xl rounded-xl border border-white/10 p-6">
            <h3 className="text-lg font-semibold text-white mb-4">{t('publicVault.sections.assets')}</h3>
            {balances.length === 0 ? (
              <p className="text-gray-400 text-center py-8">{t('publicVault.noAssets')}</p>
            ) : (
              <div className="space-y-3">
                {balances.map((token) => (
                  <div key={token.address} className="flex items-center justify-between p-4 bg-slate-700/30 rounded-lg">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <p className="text-white font-medium">{token.symbol || t('common.unknown')}</p>
      ...
      <footer className="bg-slate-800/30 border-t border-white/10 mt-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between text-sm text-gray-500">
            <span>{t('publicVault.footer.title')}</span>
            <span>{t('publicVault.footer.poweredBy')}</span>
          </div>
        </div>
      </footer>
    </div>
